...
bcc_final <- readRDS(file = "../data-ext/bcc_final_model.rds")
# kmeans results
set.seed(649)
...
```{r bcc, include=internal}
# optimal number of clusters
alpha_adjust <- lapply(bcc_fits, function(x) x$alpha.adjust) |>  unlist()
bcc_k <- which.max(alpha_adjust) + 1

# extract clusters and posterior probability from final model
bcc_dat <- bcc_final$dat
bcc_dat_uq <- bcc_dat[[1]][!duplicated(bcc_dat[[1]]$id.org, fromLast = TRUE), ]
bcc_dat_uq$post_prob <- bcc_final$postprob

bcc_n <- brain_vol |> dplyr::pull(subj_id_numeric) |> unique() |> length()
bcc_per <- paste(round(100*table(bcc_dat_uq$cluster.global)/bcc_n,1),"%",sep="")

bcc_cluster_labels <- factor(
  bcc_dat_uq$cluster.global, 
  label = glue::glue(
    "Cluster {1:bcc_k} ({bcc_per})"
  )
)

bcc_cluster <- brain_vol |> 
  dplyr::left_join(
    x = _,
    y = data.frame(
      subj_id_numeric = bcc_dat_uq$id.org,
      cluster = bcc_cluster_labels
    ),
    by = "subj_id_numeric"
)

bcc_cluster_long <- bcc_cluster |> 
  dplyr::select(
    subj_id_numeric, visit, cluster, 
    Type2.L3.Frontal_L_prop, Type2.L3.Frontal_R_prop, 
    Type2.L3.Parietal_L_prop, Type2.L3.Parietal_R_prop,
...
# plot region volumes by cluster
bcc_cluster_volumes <- bcc_cluster_long |> 
  ggplot() +
  aes(x = factor(visit), y = volume, color = cluster, group = cluster) +
  geom_smooth(method = "loess", se = FALSE, span = 1.5, linewidth = 2.5) +
  scale_y_continuous(expand = expansion(mult = 0.25)) +
...
bcc_dat_uq |> 
  ggplot() +
  aes(
    x = factor(cluster.global), y = post_prob
  ) +
  geom_boxplot() +
  labs(
    x = "Cluster", y = "Posterior Cluster Probability", 
...
```{r bcc-cluster-vol, fig.height = 12, fig.width=10, warning=FALSE, message=FALSE}
bcc_cluster_volumes
```
...
# separate version without cluster label
kml_clusters <- kml_data |> 
  dplyr::select(subj_id_numeric, cluster) |> 
  dplyr::mutate(
    kml_cluster = dplyr::case_when(
      cluster == "Cluster 1 (57.6%)" ~ 1,
      cluster == "Cluster 2 (42.4%)" ~ 2
    )
  ) |> 
...
brain_clusters <- brain_lcmm_uq |> 
  ...
  dplyr::left_join(
    ...
    y = data.frame(
      subj_id_numeric = bcc_dat_uq$id.org,
      bcc_cluster = bcc_dat_uq$cluster.global
    ),
    by = "subj_id_numeric"
  ) |> 
  dplyr::left_join(
...
lcmm_vs_bcc <- gtsummary::tbl_cross(
  brain_clusters, row = lcmm_cluster, col = bcc_cluster,
  label = list(lcmm_cluster = "LCMM", bcc_cluster = "BCC")
) |> 
  gtsummary::bold_labels()

lcmm_vs_kml <- gtsummary::tbl_cross(
  brain_clusters, row = lcmm_cluster, col = kml_cluster,
  label = list(lcmm_cluster = "LCMM", kml_cluster = "KML")
) |> 
  gtsummary::bold_labels()

bcc_vs_kml <- gtsummary::tbl_cross(
  brain_clusters, row = bcc_cluster, col = kml_cluster,
  label = list(bcc_cluster = "BCC", kml_cluster = "KML")
) |> 
  gtsummary::bold_labels()

# summary of volumes by cluster assignment
lcmm_cluster_vols <- brain_clusters_vol |> 
  gtsummary::tbl_strata(
    ...
    .tbl_fun = ~ .x |> 
      gtsummary::tbl_summary(
        ...
        statistic = gtsummary::all_continuous2() ~ c("{median} ({p25}, {p75})",
                                                     "{min} - {max}"),
        include = dplyr::all_of(brain_regions),
        label = list(
          "Type2.L3.Frontal_L_prop" = "Frontal, Left", 
          "Type2.L3.Frontal_R_prop" = "Frontal, Right", 
          "Type2.L3.Parietal_L_prop" = "Parietal, Left", 
          "Type2.L3.Parietal_R_prop" = "Parietal, Right",
          "Type2.L3.Occipital_L_prop" = "Occipital, Left", 
          "Type2.L3.Occipital_R_prop" = "Occipital, Right", 
          "Type2.L3.Temporal_L_prop" = "Temporal, Left", 
          "Type2.L3.Temporal_R_prop" = "Temporal, Right"
        )
      )
  ) |> 
  gtsummary::modify_caption(
    caption = "Proportional brain volumes over time by LCMM cluster assignment"
  )

bcc_cluster_vols <- brain_clusters_vol |> 
  gtsummary::tbl_strata(
    strata = bcc_cluster,
    .tbl_fun = ~ .x |> 
      gtsummary::tbl_summary(
  ...
  gtsummary::modify_caption(
    caption = "Proportional brain volumes over time by BCC cluster assignment"
  )

kml_cluster_vols <- brain_clusters_vol |> 
  gtsummary::tbl_strata(
    strata = kml_cluster,
    .tbl_fun = ~ .x |> 
      gtsummary::tbl_summary(
  ...
  gtsummary::modify_caption(
    caption = "Proportional brain volumes over time by KML cluster assignment"
  )
```
